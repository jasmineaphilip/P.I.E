...
def create_group_test():
	...
	sock.sendto(packet.formatClientData(token, "now", "my basement", "100", "John Smith", "mygroup"), (ip, port))
	data = sock.recvfrom(1024)
	print (data[0])
...
def report_issue_test():
	...
	sock.sendto(packet.formatClientData(token, "oops", "there was a bug"), (ip, port))
	data = sock.recvfrom(1024)
	print (data[0])
